...
label battle_def4:#you usually call this in the very beginning of your game, it will set up all the variables being used for the battle system
...
#################################################################################################################
label battle4: # the battle screen uses this general set up
    play music "battle.mp3"
    show screen battle_overlay_players4
    show screen battle_overlay_monsters4
    show screen battle_message4
    jump battling4
label battling4:
    call player_turn4 from _call_player_turn4
    $ turn = 0 #this is so that player 1 is not 'selected', that wouldn't make sense when it's not their turn
    if check_win == True:
        jump battle_win4
    call monster_turn4 from _call_monster_turn4
    $ m_turn = 1
    $ turn = 1
    $ player1_defend = False
    $ player2_defend = False
...
label battle_win4: #the aftermath message
    #merkel "¡Tú ganar!"
    ...
    $ m_turn = 0
    $ player_turn = "none"
    hide screen battle_overlay_players4
    hide screen battle_overlay_monsters4
    hide screen battle_message4
    hide screen player_actions4
    return
    ...
    while player_numbers >= turn:
        if turn == 1:
            if player1_dead:
                ...
                $ player_turn = player1
                call screen player_actions4
                $ p_action = _return
                call player_dealt_damage4 from _call_player_dealt_damage4
        elif turn == 2:
            if player2_dead:
                ...
                call screen player_actions4
                $ p_action = _return
                call player_dealt_damage4 from _call_player_dealt_damage4_1
        elif turn == 3:
            if player3_dead:
                ...
                call screen player_actions4
                $ p_action = _return
                call player_dealt_damage4 from _call_player_dealt_damage4_2
        elif turn == 4:
            if player4_dead:
                ...
                call screen player_actions4
                $ p_action = _return
                call player_dealt_damage4 from _call_player_dealt_damage4_3
        #call screen player_actions  
        #$ p_action = _return
        #call player_dealt_damage
        call monster_dead_check4 from _call_monster_dead_check4
        call player_dead_check4 from _call_player_dead_check4
        if check_win:
            jump battle_win4
        else:
            $ turn += 1
...
label player_dealt_damage4:
    $ target = "none"
    if player_turn == player1:
        if p_action == "attack":
            $ damage = player1_attack
        elif p_action == "skills":
            call player1_skills4 from _call_player1_skills4
        ...
        if p_action == "attack":
            ...
            call player2_skills4 from _call_player2_skills4
        elif p_action == "defend":
            $ damage = -1
            $ player2_defend = True
        ...
        if p_action == "attack":
            ...
            call player3_skills4 from _call_player3_skills4 
        elif p_action == "defend":
            $ damage = -1
            $ player3_defend = True
        ...
        if p_action == "attack":
            ...
            call player4_skills4 from _call_player4_skills4
        elif p_action == "defend":
            $ damage = -1
            $ player4_defend = True
    if damage < 0:
        ...
        call screen player_target4
        $ target = _return
        if target == "monster1":
            $ monster1_hp = monster1_hp - damage
            call monster_dead_check4 from _call_monster_dead_check4_1
        elif target == "monster2":
            $ monster2_hp = monster2_hp - damage
            call monster_dead_check4 from _call_monster_dead_check4_2
        elif target == "monster3":
            $ monster3_hp = monster3_hp - damage
            call monster_dead_check4 from _call_monster_dead_check4_3
        elif target == "all":
            $ monster1_hp = monster1_hp - damage
            $ monster2_hp = monster2_hp - damage
            $ monster3_hp = monster3_hp - damage
            call monster_dead_check4 from _call_monster_dead_check4_4
        if p_action == "attack":
...
label player1_skills4:
    hide screen battle_overlay_players4
    call screen noel_skills4
    $ p_skill = _return
...
label player2_skills4:
    hide screen battle_overlay_players4
    call screen kuroi_skills4
    $ p_skill = _return
...
label player3_skills4:
    hide screen battle_overlay_players4
    call screen shiro_skills4
    $ p_skill = _return
...
label player4_skills4:
    hide screen battle_overlay_players4
    call screen luna_skills4
    $ p_skill = _return
    ...
    while monster_numbers >= m_turn:
        if m_turn == 1:
            if monster1_dead:
                ...
                call player_dead_check4 from _call_player_dead_check4_1
                $ monster_turn = "monster1"
                call monster_dealt_damage4 from _call_monster_dealt_damage4
                $ message = "monster1_attack"
        elif m_turn == 2:
            if monster2_dead:
                pass
            else:
                call player_dead_check4 from _call_player_dead_check4_2
                $ monster_turn = "monster2"
                call monster_dealt_damage4 from _call_monster_dealt_damage4_1
                $ message = "monster2_attack"
        elif m_turn == 3:
            if monster3_dead:
                pass
            else:
                call player_dead_check4 from _call_player_dead_check4_3
                $ monster_turn = "monster3"
                call monster_dealt_damage4 from _call_monster_dealt_damage4_2
...
label player_dead_check4:
    ...
    if player1_hp <= 0 and player2_hp <= 0 and player3_hp <= 0 and player4_hp <= 0:
        ...
        return
screen battle_overlay_players4:
...
screen battle_overlay_monsters4:
    if monster1 != "none":
...
screen player_actions4: #returns for player action
...
screen player_target4: #returns monster player wants to attack
    if target == "all":
...
label battle_lose4: #the aftermath message
    merkel "Ahora que estar bien blandito, venir, yo tener dildo de goma para tí."
    k "Dios mío, ¿quién me mandaría a mí ser guapo..."
    screen black
    $ puntosj +=3
    hide screen battle_overlay_players4
    hide screen battle_overlay_monsters4
