...
image bb = im.FactorScale("images/gymmallorca.jpg", 1.0)
init python:
    #custom bar HP
...
label battle_def2:#you usually call this in the very beginning of your game, it will set up all the variables being used for the battle system
...
#    $ player4_dead = False
    ...
    $ monster1_image = "battle2/sheep1.png"
    ...
    $ monster1_attack_min = 15
    $ monster1_attack_max = 36
...
#################################################################################################################
label battle2: # the battle screen uses this general set up
    play music "battle.mp3"
    show screen battle_overlay_players2
    show screen battle_overlay_monsters2
    show screen battle_message2
    jump battling2
label battling2:
    call player_turn2 from _call_player_turn2
    $ turn = 0 #this is so that player 1 is not 'selected', that wouldn't make sense when it's not their turn
    if check_win == True:
        jump battle_win
    call monster_turn2 from _call_monster_turn2
    $ m_turn = 1
    $ turn = 1
    $ player1_defend = False
    $ player2_defend = False
...
label battle_win2: #the aftermath message
    merkel "¡Tú ganar!"
    merkel "Yo nunca pensar que alguien con pelo tan graso poder ser tan ágil. Tú darme secreto para la anticaida."
    c "Haberlo pensado antes de acabar con Chiripas."
    merkel "¿Quién ser Chiripas y por qué hablar tanto de él?"
    extend " ¿Ser tu mascota?"
    c "No te rías de mí o me veré obligado a acabar con tu régimen de terror. Por ahora me conformaré con tu voto."
    merkel "Aquí lo tienes, pero ir rápido. Empezar a oler a pachuli e incienso."
    merkel "Yo ser alérgica."
    $ turn = 0
    $ puntosj += 3 #los puntos que recibes al ganar
    $ m_turn = 0
    $ player_turn = "none"
    hide screen battle_overlay_players2
    hide screen battle_overlay_monsters2
    hide screen battle_message2
    hide screen player_actions2
    return
    ...
    while player_numbers >= turn:
        if turn == 1:
            if player1_dead:
                ...
                $ player_turn = player1
                call screen player_actions2  
                $ p_action = _return
                call player_dealt_damage2 from _call_player_dealt_damage2
        elif turn == 2:
            if player2_dead:
                ...
                call screen player_actions2  
                $ p_action = _return
                call player_dealt_damage2 from _call_player_dealt_damage2_1
        elif turn == 3:
            if player3_dead:
                ...
                call screen player_actions2  
                $ p_action = _return
                call player_dealt_damage2 from _call_player_dealt_damage2_2
        elif turn == 4:
            if player4_dead:
                ...
                call screen player_actions2  
                $ p_action = _return
                call player_dealt_damage2 from _call_player_dealt_damage2_3
        #call screen player_actions  
        #$ p_action = _return
        #call player_dealt_damage
        call monster_dead_check2 from _call_monster_dead_check2
        call player_dead_check2 from _call_player_dead_check2
        if check_win:
            jump battle_win2
        else:
            $ turn += 1
...
label player_dealt_damage2:
    $ target = "none"
    if player_turn == player1:
        if p_action == "attack":
            $ damage = player1_attack
        elif p_action == "skills":
            call player1_skills2 from _call_player1_skills2
        ...
        if p_action == "attack":
            ...
            call player2_skills2 from _call_player2_skills2
        elif p_action == "defend":
            $ damage = -1
            $ player2_defend = True
        ...
        if p_action == "attack":
            ...
            call player3_skills2 from _call_player3_skills2 
        elif p_action == "defend":
            $ damage = -1
            $ player3_defend = True
        ...
        if p_action == "attack":
            ...
            call player4_skills2 from _call_player4_skills2
        elif p_action == "defend":
            $ damage = -1
            $ player4_defend = True
    if damage < 0:
        ...
        call screen player_target2
        $ target = _return
        if target == "monster1":
            $ monster1_hp = monster1_hp - damage
            call monster_dead_check2 from _call_monster_dead_check2_1
        elif target == "monster2":
            $ monster2_hp = monster2_hp - damage
            call monster_dead_check2 from _call_monster_dead_check2_2
        elif target == "monster3":
            $ monster3_hp = monster3_hp - damage
            call monster_dead_check2 from _call_monster_dead_check2_3
        elif target == "all":
            $ monster1_hp = monster1_hp - damage
            $ monster2_hp = monster2_hp - damage
            $ monster3_hp = monster3_hp - damage
            call monster_dead_check2 from _call_monster_dead_check2_4
        if p_action == "attack":
...
label player1_skills2:
    hide screen battle_overlay_players2
    call screen noel_skills2
    $ p_skill = _return
...
label player2_skills2:
    hide screen battle_overlay_players2
    call screen kuroi_skills2
    $ p_skill = _return
...
label player3_skills2:
    hide screen battle_overlay_players2
    call screen shiro_skills2
    $ p_skill = _return
...
label player4_skills2:
    hide screen battle_overlay_players2
    call screen luna_skills2
    $ p_skill = _return
    ...
    while monster_numbers >= m_turn:
        if m_turn == 1:
            if monster1_dead:
                ...
                call player_dead_check2 from _call_player_dead_check2_1
                $ monster_turn = "monster1"
                call monster_dealt_damage2 from _call_monster_dealt_damage2
                $ message = "monster1_attack"
        elif m_turn == 2:
            if monster2_dead:
                pass
            else:
                call player_dead_check2 from _call_player_dead_check2_2
                $ monster_turn = "monster2"
                call monster_dealt_damage2 from _call_monster_dealt_damage2_1
                $ message = "monster2_attack"
        elif m_turn == 3:
            if monster3_dead:
                pass
            else:
                call player_dead_check2 from _call_player_dead_check2_3
                $ monster_turn = "monster3"
                call monster_dealt_damage2 from _call_monster_dealt_damage2_2
...
label player_dead_check2:
    ...
    if player1_hp <= 0 and player2_hp <= 0 and player3_hp <= 0 and player4_hp <= 0:
        ...
        return
screen battle_overlay_players2:
...
screen battle_overlay_monsters2:
    if monster1 != "none":
        add monster1_image xalign 0.5 yalign 0.30
        bar:
            xalign .5
            yalign .4
            style "bar_hp"
        ...
        text "[monster1_hp]/[monster1_hp_max]" xalign 0.5 yalign 0.4
    if monster2 != "none":
...
screen player_actions2: #returns for player action
...
screen player_target2: #returns monster player wants to attack
    if target == "all":
...
label battle_lose2: #the aftermath message
    merkel "¡Tú perder, siniestro vendedor ambulante de objetos mágicos inservibles!"
    c "Esto no acabará aquí. Un día, no muy lejano, otro héroe salido de la miseria que has creado aparecerá delante de tu casa para hacer que te enfrentes a tu destino."
    merkel "Ja, ja. ¿Te vas o necesitas otra paliza?"
    c "Me voy, pero ¡volveré!"
    merkel "Traer salchichón. Yo hambre después de sexo y peleas."
    hide screen battle_overlay_players2
    hide screen battle_overlay_monsters2
    hide screen battle_message2
    return
